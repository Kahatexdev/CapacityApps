...
class MaterialController extends BaseController
{
    ...
    public function filterstatusbahanbaku()
    {
        ...
        $apiUrl = $this->urlMaterial . 'statusbahanbaku/?' . http_build_query($params);
    ...
    public function cekBahanBaku($id, $idpln)
    {
        ...
        $apiUrl = $this->urlMaterial . 'cekBahanBaku/' . $model['model'];
        // Ambil data dari API
        $response = file_get_contents($apiUrl);
        $status = json_decode($response, true);
        // dd($status);
    ...
    {
        $model = $this->request->getGet('model');
        $apiUrl = $this->urlMaterial . 'cekStok/' . $model;
        $response = file_get_contents($apiUrl);
    ...
    public function cekStokStyle()
    {
        ...
        $apiUrl = $this->urlMaterial . 'cekStok/' . $model . '/' . $style;
        $response = file_get_contents($apiUrl);
        $stok = json_decode($response, true);
    ...
    public function getMU($model, $styleSize, $area, $qtyOrder)
    {
        ...
        $apiUrl = $this->urlMaterial . 'getMU/' . $model . '/' . $styleSize . '/' . $area;
    ...
    public function listPemesanan($area)
    {
        ...
        if (empty($tglPakai) && empty($pdk)) {
            ...
            $message = null;
            $rawList = $this->fetchApiData($this->urlMaterial . "listPemesanan/{$area}?tgl_pakai=" . urlencode($tglPakai) . "&searchPdk=" . urlencode($pdk));
            if (!is_array($rawList)) {
            ...
            foreach ($dataList as $key => $order) {
                ...
                if (isset($order['no_model'], $order['item_type'], $order['kode_warna'])) {
                    $styleApiUrl = $this->urlMaterial . 'getStyleSizeByBb?no_model='
                    ...
                    if ($styleList) {
                        ...
                        $tambahanApiUrl = $this->urlMaterial . 'getKgTambahan?no_model='
                    ...
                    // penerimaan benang
                    $pengirimanApiUrl = $this->urlMaterial . 'getTotalPengiriman?area=' . $area . '&no_model='
                    ...
                    // retur
                    $returApiUrl = $this->urlMaterial . 'getTotalRetur?area=' . $area . '&no_model='
                        . $order['no_model'] . '&item_type=' . urlencode($order['item_type']) . '&kode_warna=' . urlencode($order['kode_warna']);
        ...
        $masterRangeApiUrl = $this->urlMaterial . 'getMasterRangePemesanan?day=' .    ($day) . '&area=' . urlencode($area);
    ...
    public function filterStockBahanBaku($area)
    {
        ...
        // Jika search ada, panggil API eksternal dengan query parameter 'search'
        $apiUrl = $this->urlMaterial . 'stockbahanbaku/' . $area . '?noModel=' . urlencode($noModel) . '&warna=' . urlencode($warna);
    ...
    public function requestAdditionalTime()
    {
        ...
        // Jika search ada, panggil API eksternal dengan query parameter 'search'
        $apiUrl = $this->urlMaterial . 'requestAdditionalTime/' . $area . '?jenis=' . urlencode($jenis) . '&tanggal_pakai=' . urlencode($tanggal_pakai) . '&alasan=' . urlencode($alasan);
    ...
    public function filterPPH($area)
    {
        ...
        // Jika search ada, panggil API eksternal dengan query parameter 'search'
        $apiUrl = $this->urlMaterial . 'pph?model=' . urlencode($noModel);
    ...
    {
        $detail = [];
        $apiUrl = $this->urlMaterial . 'poTambahanDetail/' . $noModel . '/' . $area;
    ...
    public function savePoTambahan($area)
    {
        ...
        $payload = ['items' => $items];
        $apiUrl = $this->urlMaterial . 'savePoTambahan';
    ...
    public function filterTglPakai($area)
    {
        ...
        $apiUrl = $this->urlMaterial . "filterTglPakai/"
            . $area
    ...
    public function reportPemesanan($area)
    {
        ...
        $dataList = fetchApiData($this->urlMaterial . "listReportPemesanan/" . $area . "/" . urlencode($tgl_pakai));
    ...
    public function getNomodel()
    {
        ...
        if ($poTambahan == 1) {
            // Jika search ada, panggil API eksternal dengan query parameter 'search'
            $apiUrl = $this->urlMaterial . 'getNoModelByPoTambahan?area=' . $area;
    ...
    public function getStyleSizeByNoModelPemesanan()
    {
        ...
        if ($poTambahan == 1) {
            // Jika search ada, panggil API eksternal dengan query parameter 'search'
            $apiUrl = $this->urlMaterial . 'getStyleSizeByPoTambahan?no_model=' . $noModel . '&area=' . $area;
    ...
    public function sisaKebutuhanArea($area)
    {
        ...
        if (!empty($area) && !empty($noModel)) {
            ...
            $pemesananUrl  = $this->urlMaterial . 'getPemesananByAreaModel?area=' . $area . '&no_model=' . urlencode($noModel);
            $pemesananResponse = file_get_contents($pemesananUrl);
            $dataPemesanan   = json_decode($pemesananResponse, true);

            $returUrl  = $this->urlMaterial . 'getReturByAreaModel?area=' . $area . '&no_model=' . urlencode($noModel);
        ...
        foreach ($dataPemesanan as $key => $pemesanan) {
            ...
            $urlStyle = $this->urlMaterial . 'getStyleSizeByBb?no_model=' . $pemesanan['no_model']
            ...
            foreach ($getStyle as $i => $data) {
                ...
                // $PoPlus = $this->urlMaterial . 'getKgPoTambahan?no_model=' . $pemesanan['no_model']
                //     . '&item_type=' . urlencode($pemesanan['item_type'])
                //     . '&area=' . $area;
            ...
            // Ambil kg po tambahan
            $PoPlus = $this->urlMaterial . 'getKgTambahan?no_model=' . $pemesanan['no_model']
                . '&item_type=' . urlencode($pemesanan['item_type'])
                . '&kode_warna=' . urlencode($pemesanan['kode_warna'])
                . '&area=' . $area;
        ...
        foreach ($dataRetur as $retur) {
            // ambil data styleSize by bb
            $urlStyle = $this->urlMaterial . 'getStyleSizeByBb?no_model=' . $retur['no_model']
            ...
            foreach ($getStyle as $i => $data) {
                ...
                // // Ambil kg po tambahan
                // $PoPlus = $this->urlMaterial . 'getKgPoTambahan?no_model=' . $retur['no_model']
                //     . '&item_type=' . urlencode($retur['item_type'])
                //     . '&area=' . $area;
            ...
            // // Ambil kg po tambahan
            $PoPlus = $this->urlMaterial . 'getKgTambahan?no_model=' . $retur['no_model']
                . '&item_type=' . urlencode($retur['item_type'])
                . '&kode_warna=' . urlencode($retur['kode_warna'])
                . '&area=' . $area;
    ...
    public function filterDatangBenang()
    {
        ...
        $apiUrl = $this->urlMaterial . 'filterDatangBenang?key=' . urlencode($key) . '&tanggal_awal=' . $tanggalAwal . '&tanggal_akhir=' . $tanggalAkhir . '&po_plus=' . $poPlus;
        $material = @file_get_contents($apiUrl);
    ...
    public function filterPoBenang()
    {
        ...
        $apiUrl = $this->urlMaterial . 'filterPoBenang?key=' . urlencode($key);
        $material = @file_get_contents($apiUrl);
    ...
    public function filterPengiriman()
    {
        ...
        $apiUrl = $this->urlMaterial . 'filterPengiriman?key=' . urlencode($key) . '&tanggal_awal=' . urlencode($tanggalAwal) . '&tanggal_akhir=' . urlencode($tanggalAkhir);
        $material = @file_get_contents($apiUrl);
    ...
    public function filterReportGlobal()
    {
        ...
        $apiUrl = $this->urlMaterial . 'filterReportGlobal?key=' . urlencode($key) . '&jenis=' . urlencode($jenis);
        $material = @file_get_contents($apiUrl);
    ...
    public function filterSisaPakai()
    {
        ...
        $apiUrl = $this->urlMaterial . 'filterSisaPakai?bulan=' . urlencode($bulan) . '&no_model=' . urlencode($noModel) . '&kode_warna=' . urlencode($kodeWarna) . '&jenis=' . urlencode($jenis);
        $material = @file_get_contents($apiUrl);
    ...
    public function historyPindahOrder()
    {
        ...
        // 1) Ambil data
        $apiUrl = $this->urlMaterial . 'historyPindahOrder?model=' . urlencode($noModel) . '&kode_warna=' . urlencode($kodeWarna);
        $material = @file_get_contents($apiUrl);
    ...
    public function reportSisaDatangBenang()
    {
        ...
        $apiUrl = $this->urlMaterial . 'reportSisaDatangBenang?delivery=' . urlencode($bulan) . '&no_model=' . urlencode($noModel) . '&kode_warna=' . urlencode($kodeWarna);
        $material = @file_get_contents($apiUrl);
    ...
    public function reportSisaDatangNylon()
    {
        ...
        $bulan = $bulanMap[$delivery] ?? null;
        $apiUrl = $this->urlMaterial . 'reportSisaDatangNylon?delivery=' . urlencode($bulan) . '&no_model=' . urlencode($noModel) . '&kode_warna=' . urlencode($kodeWarna);
        $material = @file_get_contents($apiUrl);
    ...
    public function reportSisaDatangSpandex()
    {
        ...
        $apiUrl = $this->urlMaterial . 'reportSisaDatangSpandex?delivery=' . urlencode($bulan) . '&no_model=' . urlencode($noModel) . '&kode_warna=' . urlencode($kodeWarna);
        $material = @file_get_contents($apiUrl);
    ...
    public function reportSisaDatangKaret()
    {
        ...
        $apiUrl = $this->urlMaterial . 'reportSisaDatangKaret?delivery=' . urlencode($bulan) . '&no_model=' . urlencode($noModel) . '&kode_warna=' . urlencode($kodeWarna);
        $material = @file_get_contents($apiUrl);
    ...
    public function filterBenangMingguan()
    {
        ...
        $apiUrl = $this->urlMaterial . 'filterBenangMingguan?tanggal_awal=' . urlencode($tanggalAwal) . '&tanggal_akhir=' . urlencode($tanggalAkhir);
        $material = @file_get_contents($apiUrl);
    ...
    public function filterBenangBulanan()
    {
        ...
        $apiUrl = $this->urlMaterial . 'filterBenangBulanan?tanggal_awal=' . urlencode($tanggalAwal) . '&tanggal_akhir=' . urlencode($tanggalAkhir);
        $material = @file_get_contents($apiUrl);
    ...
    public function stockarea($area)
    {
        ...
        $apiUrl = $this->urlMaterial . 'countKirimArea/' . $area . '/' . $tgl;
    ...
    public function inStock($area)
    {
        ...
        if ($tgl) {
            $apiUrl = $this->urlMaterial . 'getListKirimArea/' . $area . '/' . $tgl;
            $response = @file_get_contents($apiUrl);
    ...
    public function saveStock()
    {
        ...
        $apiUrl = $this->urlMaterial . 'updateTerimaArea/' . $idPengeluaran;
