...
class RossoController extends BaseController
{
    ...
    public function getMU($model, $styleSize, $area, $qtyOrder)
    {
        ...
        $apiUrl = api_url('material') . 'getMUForRosso/' . $model . '/' . $styleSize . '/' . $area;
    ...
    public function listPemesanan($area)
    {
        ...
        if (empty($tglPakai) && empty($pdk)) {
            ...
            $rawList = fetchApiData(api_url('material') . "listPemesanan/$area");
            if (!is_array($rawList)) {
        ...
        // Fetch master range
        $masterRangeApiUrl = api_url('material') . 'getMasterRangePemesanan?day='
    ...
    public function requestAdditionalTime()
    {
        ...
        // Jika search ada, panggil API eksternal dengan query parameter 'search'
        $apiUrl = api_url('material') . 'requestAdditionalTime/' . $area . '?jenis=' . urlencode($jenis) . '&tanggal_pakai=' . urlencode($tanggal_pakai);
    ...
    public function reportPemesanan($area)
    {
        ...
        $dataList = fetchApiData(api_url('material') . "listReportPemesanan/$area/$tglPakai");
    ...
    public function sisaKebutuhanArea($area)
    {
        ...
        foreach ($dataRetur as $retur) {
            ...
            foreach ($getStyle as $i => $data) {
                ...
                $PoPlus = api_url('material') . 'getKgPoTambahan?no_model=' . $retur['no_model']
                    . '&item_type=' . urlencode($retur['item_type'])
                    . '&area=' . $area;
