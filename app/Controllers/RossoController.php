...
class RossoController extends BaseController
{
    ...
    public function getMU($model, $styleSize, $area, $qtyOrder)
    {
        ...
        $apiUrl = $this->urlMaterial . 'getMUForRosso/' . $model . '/' . $styleSize . '/' . $area;
    ...
    public function listPemesanan($area)
    {
        ...
        if (empty($tglPakai) && empty($pdk)) {
            ...
            $rawList = fetchApiData($this->urlMaterial . "listPemesanan/$area");
            if (!is_array($rawList)) {
        ...
        // Fetch master range
        $masterRangeApiUrl = $this->urlMaterial . 'getMasterRangePemesanan?day='
    ...
    public function requestAdditionalTime()
    {
        ...
        // Jika search ada, panggil API eksternal dengan query parameter 'search'
        $apiUrl = $this->urlMaterial . 'requestAdditionalTime/' . $area . '?jenis=' . urlencode($jenis) . '&tanggal_pakai=' . urlencode($tanggal_pakai);
    ...
    public function reportPemesanan($area)
    {
        ...
        $dataList = fetchApiData($this->urlMaterial . "listReportPemesanan/$area/$tglPakai");
